<div
	class="flex w-full flex-col items-center justify-center space-y-4 bg-[#36636c]/40 px-4 py-12 dark:text-gray-200"
>
	<p class="text-xl font-bold tracking-wide">Come work with us!</p>
	<!-- <p>
		Please click <a
			href="/PDFs/Development-Director.pdf"
			target="_blank"
			class="font-bold underline underline-offset-4 transition-all duration-350 hover:opacity-75"
			>here</a
		>
		to read about the <b>Development Director</b> position.
	</p> -->
	<!-- <p>
		And click <a
			href="/PDFs/Indigenous-Community-Liaison.pdf"
			target="_blank"
			class="font-bold underline underline-offset-4 transition-all duration-350 hover:opacity-75"
			>here</a
		>
		to learn more about our <b>Indigenous Community Liaison</b> role.
	</p> -->
	<p>
		We are accepting proposals for <a
			href="/PDFs/CateringRFP2026.pdf"
			target="_blank"
			class="font-bold underline underline-offset-4 transition-all duration-350 hover:opacity-75"
			>Catering Services for the Wallowa Gathering</a
		> in May.
	</p>
</div>
